...
#
# We will later apply regularization to avoid the potential for overfitting the
# data.
...
def dcost(parameters):
	predicted = sigmoid(features.dot(parameters))
	difference = predicted - accepted
	out = np.transpose(features).dot(difference) / num_samples
	return out
